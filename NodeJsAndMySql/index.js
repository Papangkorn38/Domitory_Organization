...
app.post('/upload', upload.single('IMG'), (req, res) => {
    ...
    if (!req.file) {
        ...
        // ใช้ uploadedFileName (หรือ uploadedFilePath) แทน req.body.IMG
        connection.query(inserDATA, [PID,CID,RoomID,uploadedFileName], (error, result) => {
            if (error) {
                console.error(error);
                res.status(500).send("Database error");
                return;
...
app.patch('/api/update/client/:roomid',async(req,res) => {
    ...
    const { CID , Fname , Lname , Username , Password , Cemail , Address , BirthDate , CitizenID , PhoneNum , RoomID } = req.body;
    const query = "UPDATE client SET CID = ? ,Fname = ? , Lname = ?,Username = ?, Password = ?,Cemail = ?, Address = ?, BirthDate = ? ,CitizenID = ?, PhoneNum = ?, RoomID = ? WHERE RoomID = ?"
    try{
        connection.query(query, [CID , Fname , Lname , Username , Password , Cemail , Address , BirthDate , CitizenID , PhoneNum , RoomID,roomID], (error,results,fields)=>{
            if(error){
                console.log(error);
...
app.patch('/api/update/room/:roomid',async(req,res) => {
    ...
    try{
        connection.query(query, [RoomID , Status ,roomID], (error,results,fields)=>{
            if(error){
                console.log(error);
                return res.status(400).send();
            }
...
app.get('/api/read/room/:RoomID',async(req,res) => {
    const roomid = req.params.RoomID;
    try{
        connection.query("SELECT * FROM room WHERE RoomID = ?", [roomid], (error,results,fields)=>{
            if(error){
                console.log(error);
                return res.status(400).send();
            }
...
app.get('/api/read/admin/:AID',async(req,res) => {
    ...
    try{
        connection.query("SELECT * FROM admin WHERE AID = ?", [adminID], (error,results,fields)=>{
            if(error){
                console.log(error);
                return res.status(400).send();
            }
...
app.get('/api/read/slip/:RoomID',async(req,res) => {
    const roomID = req.params.RoomID;
    try{
        connection.query("SELECT * FROM slip WHERE RoomID = ?", [roomID], (error,results,fields)=>{
            if(error){
                console.log(error);
                return res.status(400).send();
            }
...
app.get('/api/read/request/:RoomID',async(req,res) => {
    const roomID = req.params.RoomID;
    try{
        connection.query("SELECT * FROM request WHERE RoomID = ?", [roomID], (error,results,fields)=>{
            if(error){
                console.log(error);
                return res.status(400).send();
            }
...
app.get('/api/read/request/:RoomID/:Status',async(req,res) => {
    ...
    const roomID = req.params.RoomID;
    try{
        connection.query("SELECT * FROM request WHERE Status = ? AND RoomID = ? ", [status,roomID], (error,results,fields)=>{
            if(error){
                console.log(error);
                return res.status(400).send();
            }
